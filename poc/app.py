...
tab1, tab2, tab3 = st.tabs([
    "🔗  Quality Gate — Live PR Review  (Azure DevOps)",
...
# ══════════════════════════════════════════════════════════════════════
# TAB 1 — LIVE PR REVIEW (Azure DevOps) — Quality Gate on real PRs
# ══════════════════════════════════════════════════════════════════════
with tab1:
            ...
            if get_active_provider() == "none":
                ...
                if result:
                    ...
                    if not ascent:
                        ...
                        _raw_score = ascent.get("overall_score")
                        try:
                            overall = max(1, min(10, int(_raw_score))) if _raw_score else 0
                        except (TypeError, ValueError):
                            overall = 0
                        oc = score_colour(overall)
                        overall_display = f"{overall}/10" if overall and overall > 0 else "—"
